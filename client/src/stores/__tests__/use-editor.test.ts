...
describe("useEditor – cancelAction", () => {
  ...
  it("clears any active catalog placement", () => {
    state().beginPlacement({
      ...
      bimRef: { source: "catalog", externalId: "sofa-01" },
...
describe("useEditor – placement", () => {
  ...
  it("setPreviewPoint keeps the snapped point available during placement", () => {
    state().beginPlacement({
      ...
      previewUrl: "/assets/furniture/previews/bench-01.png",
      thumbnailUrl: "/assets/furniture/bench-01.webp",
      dimensions: { x: 1.4, y: 0.5, z: 0.5 },
      bounds: { x: 1.42, y: 0.55, z: 0.55 },
      origin: { x: 0.5, y: 0, z: 0.5 },
      qualityTier: "draft",
      styleTier: "realistic",
      materialSlots: [{ slotId: "primary", label: "Primary finish", finishId: "item-oak", finishVariantId: "natural" }],
      provenance: { source: "poly-haven", license: "CC0" },
      bimRef: { source: "catalog", externalId: "bench-01" },
      performanceBudgetKb: 500,
      keywords: ["bench"],
    });
