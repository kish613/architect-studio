...
export async function createRetextureTask(modelUrl: string, texturePrompt: string): Promise<MeshyTaskResult> {
  try {
...
export async function checkRetextureTaskStatus(taskId: string): Promise<MeshyTaskResult> {
  try {
    const response = await fetch(`${MESHY_API_URL}/openapi/v1/retexture/${taskId}`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${getMeshyApiKey()}`,
      },
...
export async function pollRetextureTask(taskId: string, maxAttempts: number = 60): Promise<MeshyTaskResult> {
  return pRetry(
    async () => {
