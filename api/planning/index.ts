...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const analyses = await db
      ...
      .where(eq(planningAnalyses.userId, session.userId))
      .orderBy(desc(planningAnalyses.createdAt));
    ...
    return res.status(500).json({ error: "Failed to fetch planning analyses" });
  }
}
